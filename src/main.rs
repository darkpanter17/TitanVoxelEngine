...
fn main() {
    ...
    let download_fn = lua
        .create_function(move |_, (url, path): (String, String)| {
    ...
    let event_loop = match EventLoop::new() {
        Ok(el) => el,
        Err(e) => {
            eprintln!("Error al crear EventLoop (¿entorno headless?): {}", e);
            return;
        }
    };

    let window = match WindowBuilder::new()
        .with_title("Titan Voxel: Textured")
        .build(&event_loop) {
        Ok(w) => w,
        Err(e) => {
            eprintln!("Error al crear Window: {}", e);
            return;
        }
    };
