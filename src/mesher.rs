...
pub fn generate_mesh(chunk: &Chunk) -> Mesh {
    ...
    for x in 0..CHUNK_SIZE {
        for z in 0..CHUNK_SIZE {
            for y in 0..CHUNK_HEIGHT { // Dibujamos hasta altura 100
...
fn push_quad(verts: &mut Vec<Vertex>, inds: &mut Vec<u32>, count: &mut u32, 
             pos: [f32; 3], dim: [f32; 2], layer: u32) {
    verts.push(Vertex { pos: [pos[0], pos[1], pos[2]], uv: [0.0, 0.0], layer });
    verts.push(Vertex { pos: [pos[0]+dim[0], pos[1], pos[2]], uv: [dim[0], 0.0], layer });
    verts.push(Vertex { pos: [pos[0]+dim[0], pos[1], pos[2]+dim[1]], uv: [dim[0], dim[1]], layer });
    verts.push(Vertex { pos: [pos[0], pos[1], pos[2]+dim[1]], uv: [0.0, dim[1]], layer });
