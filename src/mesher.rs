    ...
    for x in 0..CHUNK_SIZE {
        for z in 0..CHUNK_SIZE {
            for y in 0..CHUNK_HEIGHT {
                ...
                if voxel_id != 0 {
                    let layer = (voxel_id - 1) as u32; // ID 1 -> layer 0
                    ...
                    if y == CHUNK_HEIGHT - 1 || chunk.get_voxel(x, y + 1, z) == 0 {
                        push_quad(&mut vertices, &mut indices, &mut index_count,
                            ...
                            [xf + 1.0, yf + 1.0, zf], [xf, yf + 1.0, zf], layer);
                    }
                    ...
                    if y == 0 || chunk.get_voxel(x, y - 1, z) == 0 {
                        push_quad(&mut vertices, &mut indices, &mut index_count,
                            ...
                            [xf + 1.0, yf, zf + 1.0], [xf, yf, zf + 1.0], layer);
                    }

                    // Front (Z+)
                    if z == CHUNK_SIZE - 1 || chunk.get_voxel(x, y, z + 1) == 0 {
                        push_quad(&mut vertices, &mut indices, &mut index_count,
                            [xf, yf, zf + 1.0], [xf + 1.0, yf, zf + 1.0],
                            [xf + 1.0, yf + 1.0, zf + 1.0], [xf, yf + 1.0, zf + 1.0], layer);
                    }

                    // Back (Z-)
                    if z == 0 || chunk.get_voxel(x, y, z - 1) == 0 {
                        push_quad(&mut vertices, &mut indices, &mut index_count,
                            [xf + 1.0, yf, zf], [xf, yf, zf],
                            [xf, yf + 1.0, zf], [xf + 1.0, yf + 1.0, zf], layer);
                    }
                    ...
                    if x == CHUNK_SIZE - 1 || chunk.get_voxel(x + 1, y, z) == 0 {
                        push_quad(&mut vertices, &mut indices, &mut index_count,
                            ...
                            [xf + 1.0, yf + 1.0, zf], [xf + 1.0, yf + 1.0, zf + 1.0], layer);
                    }
                    ...
                    if x == 0 || chunk.get_voxel(x - 1, y, z) == 0 {
                        push_quad(&mut vertices, &mut indices, &mut index_count,
                            ...
                            [xf, yf + 1.0, zf + 1.0], [xf, yf + 1.0, zf], layer);
                    }
                }
            }
        }
...
fn push_quad(verts: &mut Vec<Vertex>, inds: &mut Vec<u32>, count: &mut u32, 
    ...
    verts.push(Vertex { pos: p0, uv: [0.0, 0.0], layer });
    verts.push(Vertex { pos: p1, uv: [1.0, 0.0], layer });
    verts.push(Vertex { pos: p2, uv: [1.0, 1.0], layer });
    verts.push(Vertex { pos: p3, uv: [0.0, 1.0], layer });
