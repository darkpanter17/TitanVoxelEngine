...
fn get_voxel_safe(chunk: &Chunk, x: i32, y: i32, z: i32) -> u16 {
    if x < 0 || y < 0 || z < 0 || x >= CHUNK_SIZE as i32 || y >= CHUNK_HEIGHT as i32 || z >= CHUNK_SIZE as i32 {
        return 0; // Return air if out of bounds for now
    }
    ...
    for x in 0..CHUNK_SIZE {
        for z in 0..CHUNK_SIZE {
            for y in 0..CHUNK_HEIGHT {
                 ...
                 if voxel_id != 0 {
                    let layer = voxel_id as u32 - 1; // Map ID to texture layer (0-indexed)
                    let xf = base_x + x as f32;
                    let yf = base_y + y as f32;
                    let zf = base_z + z as f32;
