    ...
    for x in 0..CHUNK_SIZE {
        for z in 0..CHUNK_SIZE {
            for y in 0..CHUNK_HEIGHT {
                ...
                if voxel != 0 {
                    let layer = (voxel - 1) as u32; // Map block ID to layer (1 -> 0, 2 -> 1, ...)
                    let x_i32 = x as i32;
                    let y_i32 = y as i32;
                    let z_i32 = z as i32;
                    ...
                    // Top face (+Y)
                    if chunk.get_voxel_safe(x_i32, y_i32 + 1, z_i32) == 0 {
                        push_face(
                    ...
                    // Left face (-X)
                    if chunk.get_voxel_safe(x_i32 - 1, y_i32, z_i32) == 0 {
                        push_face(
                            &mut vertices, &mut indices, &mut index_count,
                            [
                                [xf, yf + 1.0, zf],
                    ...
                    // Front face (+Z)
                    if chunk.get_voxel_safe(x_i32, y_i32, z_i32 + 1) == 0 {
                        push_face(
                            &mut vertices, &mut indices, &mut index_count,
                            [
                                [xf, yf + 1.0, zf + 1.0],
