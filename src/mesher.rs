...
pub fn generate_mesh(chunk: &Chunk) -> Mesh {
    ...
    // Asumimos position.y es 0 para los chunks en este engine básico
    ...
    for x in 0..CHUNK_SIZE {
        for z in 0..CHUNK_SIZE {
            for y in 0..crate::chunk::CHUNK_HEIGHT {
                ...
                if voxel != 0 {
                    let layer = voxel as u32 - 1; // Block ID 1 uses Texture Layer 0

                    let xi = x as isize;
                    let yi = y as isize;
                    let zi = z as isize;

                    let xf = x as f32 + chunk_x_offset;
                    let yf = y as f32;
                    let zf = z as f32 + chunk_z_offset;
