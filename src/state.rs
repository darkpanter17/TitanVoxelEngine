...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> anyhow::Result<Self> {
        ...
        let config = surface.get_default_config(&adapter, size.width, size.height).ok_or_else(|| anyhow::anyhow!("Failed to get surface default config"))?;
            ...
            for cz in 0..3 {
                ...
                for x in 0..32 {
                    for z in 0..32 {
                        ...
                        let h = ((wx * 0.1).sin() * 5.0 + (wz * 0.1).cos() * 5.0 + 10.0) as i32;
                        let height = h.clamp(1, 100) as usize;
                ...
                let shifted_indices = mesh.indices.iter().map(|i| i + vertex_count);
                all_indices.extend(shifted_indices);
            }
        }
