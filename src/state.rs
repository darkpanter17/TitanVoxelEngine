    ...
    pub async fn new(window: &'a Window) -> Self {
        ...
        for cx in -1..=1 {
            for cz in -1..=1 {
                ...
                for x in 0..32 {
                    for z in 0..32 {
                        let world_x = (cx * 32 + x as i32) as f32;
                        let world_z = (cz * 32 + z as i32) as f32;

                        let base_height = 10.0;
                        let height_variation = (world_x * 0.1).sin() * 5.0 + (world_z * 0.1).cos() * 5.0;
                        let max_y = (base_height + height_variation) as usize;
                        let safe_max_y = max_y.min(crate::chunk::CHUNK_HEIGHT - 1);

                        for y in 0..=safe_max_y {
                            let id = if y == safe_max_y { 2 } else { 1 }; // 2=Grass (Top), 1=Dirt (Bottom)
