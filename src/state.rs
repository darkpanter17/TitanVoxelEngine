...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> anyhow::Result<Self> {
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            for x in 0..32 {
                for z in 0..32 {
                    let global_x = (chunk.position.x * 32) as f32 + x as f32;
                    let global_z = (chunk.position.z * 32) as f32 + z as f32;

                    let height = ((global_x * 0.1).sin() * 5.0 + (global_z * 0.1).cos() * 5.0) as i32 + 10;
