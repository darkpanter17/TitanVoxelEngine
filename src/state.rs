...
pub const TEXTURE_PATHS: [&str; 1] = [
    "assets/textures/1.png", // Texture Atlas
];
    ...
    pub async fn new(window: &'a Window) -> anyhow::Result<Self> {
        ...
        for cx in -1..=1 {
            for cz in -1..=1 {
                ...
                for x in 0..32 {
                    for z in 0..32 {
                        let wx = cx as f32 * 32.0 + x as f32;
                        let wz = cz as f32 * 32.0 + z as f32;
                        let height = (10.0 + (wx * 0.1).sin() * 5.0 + (wz * 0.1).cos() * 5.0) as usize;
