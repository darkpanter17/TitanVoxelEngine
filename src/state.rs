...
use crate::{mesher::{self, Vertex, Mesh}, chunk::Chunk, texture, camera};
use glam::{IVec3, Vec3};
use rayon::prelude::*;
...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> anyhow::Result<Self> {
        ...
        let meshes: Vec<Mesh> = chunks.par_iter().map(mesher::generate_mesh).collect();
