...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> anyhow::Result<Self> {
        ...
        let texture_array = match texture::Texture::load_texture_array(&device, &queue, texture_paths_vec) {
            Ok(t) => t,
            Err(e) => {
        ...
        // --- CAMERA ---
        let camera = camera::Camera { eye: Vec3::new(-5.0, 50.0, -5.0), target: Vec3::new(1.0, -0.5, 1.0).normalize(), up: Vec3::Y, aspect: config.width as f32 / config.height as f32, fovy: 45.0f32.to_radians(), znear: 0.1, zfar: 1000.0, yaw: -45.0f32.to_radians(), pitch: -20.0f32.to_radians() };
            ...
            for cz in -1..=1 {
                ...
                for x in 0..32 {
                    for z in 0..32 {
                        ...
                        // Height map generation (Sine waves)
                        let height = 10 + (
                            (world_x / 20.0).sin() * 5.0 +
                            (world_z / 20.0).cos() * 5.0
                        ) as i32;

                        for y in 0..=height {
                            if y >= 256 { break; }
                            // 1 = Grass (Top layer), 2 = Dirt (Below)
                            let id = if y == height { 2 } else { 1 };
                            chunk.set_voxel(x as usize, y as usize, z as usize, id);
                        }
                    }
                }
