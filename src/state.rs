...
pub struct State<'a> {
    ...
    // En lugar de buffers singulares, ahora un HashMap para soportar múltiples chunks
...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
            ...
            for cz in -1..=1 {
                ...
                for x in 0..CHUNK_SIZE {
                    for z in 0..CHUNK_SIZE {
                        let world_x = (cx * CHUNK_SIZE as i32) as f32 + x as f32;
                        let world_z = (cz * CHUNK_SIZE as i32) as f32 + z as f32;
                        let height = 10 + ((world_x * 0.1).sin() * 3.0) as i32 + ((world_z * 0.1).cos() * 3.0) as i32;
                        for y in 0..=height {
                            let id = if y == height { 2 } else { 1 }; // 2=Grass (Top), 1=Dirt (Bottom)
                            chunk.set_voxel(x, y as usize, z, id);
                        }
                    }
                }
        ...
        let meshes: Vec<_> = world.chunks.values()
            .collect::<Vec<_>>()
            .par_iter()
            .map(|chunk| (chunk.position, mesher::generate_mesh(chunk)))
            .collect();

        let mut chunk_buffers = HashMap::new();
        for (pos, mesh) in meshes {
            if mesh.indices.is_empty() { continue; }
            let vb = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some(&format!("VB {:?}", pos)), contents: bytemuck::cast_slice(&mesh.vertices), usage: wgpu::BufferUsages::VERTEX });
            let ib = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some(&format!("IB {:?}", pos)), contents: bytemuck::cast_slice(&mesh.indices), usage: wgpu::BufferUsages::INDEX });
            chunk_buffers.insert(pos, (vb, ib, mesh.indices.len() as u32));
        }
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Render Encoder") });
        {
            ...
            for (vb, ib, idx_count) in self.chunk_buffers.values() {
