...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        let (device, queue) = adapter.request_device(&wgpu::DeviceDescriptor { label: Some("Device"), required_features: wgpu::Features::TEXTURE_BINDING_ARRAY | wgpu::Features::SAMPLED_TEXTURE_AND_STORAGE_BUFFER_ARRAY_NON_UNIFORM_INDEXING, required_limits: wgpu::Limits::downlevel_webgl2_defaults() }, None).await.unwrap();
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            let offset_x = chunk.position.x * CHUNK_SIZE as i32;
            let offset_z = chunk.position.z * CHUNK_SIZE as i32;
            ...
            for x in 0..CHUNK_SIZE {
                for z in 0..CHUNK_SIZE {
                    ...
                    let height_val = base_height + (world_x as f32 / 5.0).sin() * 5.0 + (world_z as f32 / 5.0).cos() * 5.0;
                    let surface_y = (height_val as usize).min(CHUNK_HEIGHT - 1);

                    for y in 0..=surface_y {
                        let id = if y == surface_y { 1 } else { 2 }; // 1=Grass, 2=Dirt
