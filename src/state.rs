...
use crate::{mesher::{self, Vertex}, chunk::Chunk, texture, camera, world::World};
use glam::{IVec3, Vec3};
...
pub struct State<'a> {
    ...
    chunks: HashMap<IVec3, (wgpu::Buffer, wgpu::Buffer, u32)>,
...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        let texture_paths: Vec<String> = TEXTURE_PATHS.iter().map(|&s| s.to_string()).collect();
        ...
        for cx in -1..=1 {
            for cz in -1..=1 {
                ...
                for x in 0..32 {
                    for z in 0..32 {
                        let gx = cx * 32 + x as i32;
                        let gz = cz * 32 + z as i32;
                        let h = (((gx as f32 * 0.1).sin() * 5.0 + (gz as f32 * 0.1).cos() * 5.0) as i32 + 10).clamp(0, 255) as usize;
                        for y in 0..=h {
                            let id = if y == h { 1 } else { 2 }; // 1=Grass (Top), 2=Dirt (Bottom)
        ...
        let mut state_chunks = HashMap::new();
        let chunk_meshes: Vec<_> = world.chunks.par_iter().map(|(pos, chunk)| {
            let mut mesh = mesher::generate_mesh(chunk);

            // Adjust vertex positions by chunk offset
            let offset_x = (pos.x * 32) as f32;
            let offset_y = (pos.y * 256) as f32;
            let offset_z = (pos.z * 32) as f32;
        ...
        for (pos, mesh) in chunk_meshes {
            ...
            state_chunks.insert(pos, (vertex_buffer, index_buffer, num_indices));
        }

        let depth_texture = texture::Texture::create_depth_texture(&device, &config, "depth_texture");

        Self { window, surface, device, queue, config, size, render_pipeline, chunks: state_chunks, depth_texture, camera, camera_controller, camera_uniform, camera_buffer, camera_bind_group, diffuse_bind_group }
    }
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Render Encoder") });
        {
            ...
            for (vertex_buffer, index_buffer, num_indices) in self.chunks.values() {
