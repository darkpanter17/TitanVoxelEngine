...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            let chunk_x = chunk.position.x * CHUNK_SIZE as i32;
            let chunk_z = chunk.position.z * CHUNK_SIZE as i32;
            for x in 0..CHUNK_SIZE {
                for z in 0..CHUNK_SIZE {
                    let world_x = chunk_x + x as i32;
                    let world_z = chunk_z + z as i32;

                    let noise = ((world_x as f32 * 0.1).sin() * 5.0 + (world_z as f32 * 0.1).cos() * 5.0) as i32;
                    let surface_y = (20 + noise).max(0).min(CHUNK_HEIGHT as i32 - 1) as usize;
        ...
        for mesh in meshes {
            ...
            for index in &mesh.indices {
                all_indices.push(index + _vertex_count);
            }
        }
