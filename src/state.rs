...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            let chunk_x_offset = (chunk.position.x * 32) as f32;
            let chunk_z_offset = (chunk.position.z * 32) as f32;
            for x in 0..32 {
                for z in 0..32 {
                    let world_x = chunk_x_offset + x as f32;
                    let world_z = chunk_z_offset + z as f32;
                    let h = (f32::sin(world_x * 0.1) * f32::cos(world_z * 0.1) * 5.0) as i32 + 10;
                    let h = h.clamp(0, 255) as usize;
