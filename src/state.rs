...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        let texture_paths = TEXTURE_PATHS.iter().map(|&s| s.to_string()).collect::<Vec<String>>();
        ...
        let chunks_and_meshes: Vec<(IVec3, Chunk, mesher::Mesh)> = chunk_positions.into_par_iter().map(|pos| {
            ...
            for x in 0..CHUNK_SIZE {
                for z in 0..CHUNK_SIZE {
                    ...
                    // Simple sine wave terrain height
                    let height = (16.0 + (world_x * 0.1).sin() * 5.0 + (world_z * 0.1).cos() * 5.0) as usize;

                    for y in 0..height {
                        let id = if y == height - 1 { 1 } else { 2 }; // 1=Grass (Top), 2=Dirt (Bottom)
        ...
        for (pos, chunk, mesh) in chunks_and_meshes {
            ...
            chunk_meshes.insert(pos, (vertex_buffer, index_buffer, num_indices));
            world.insert_chunk(pos, chunk);
        }

        let depth_texture = texture::Texture::create_depth_texture(&device, &config, "depth_texture");

        Self { window, surface, device, queue, config, size, render_pipeline, chunk_meshes, depth_texture, camera, camera_controller, camera_uniform, camera_buffer, camera_bind_group, diffuse_bind_group, world }
    }
