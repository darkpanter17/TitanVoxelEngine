...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> anyhow::Result<Self> {
        ...
        let config = surface.get_default_config(&adapter, size.width, size.height).ok_or_else(|| anyhow::anyhow!("No surface config found"))?;
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            for x in 0..crate::chunk::CHUNK_SIZE {
                for z in 0..crate::chunk::CHUNK_SIZE {
                    ...
                    for y in 0..height {
                        let id = if y == height - 1 { 2 } else { 1 }; // 2=Grass, 1=Dirt
