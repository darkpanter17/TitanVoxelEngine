...
pub struct State<'a> {
    ...
    chunk_meshes: HashMap<IVec3, (wgpu::Buffer, wgpu::Buffer, u32)>,
...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        let texture_array =
            match texture::Texture::load_texture_array(&device, &queue, texture_paths.clone()) {
                Ok(t) => t,
                Err(e) => {
                    log::warn!("Texturas no cargadas: {}. Usando placeholder.", e);
                    texture::Texture::create_placeholder_texture_array(
                        &device,
                        &queue,
                        texture_paths.len() as u32,
                    )
                }
            };
        ...
        let camera = camera::Camera {
            eye: Vec3::new(-5.0, 10.0, -5.0),
            target: Vec3::new(1.0, -0.5, 1.0).normalize(),
            up: Vec3::Y,
            ...
            for cz in -1..=1 {
                ...
                for x in 0..CHUNK_SIZE {
                    for z in 0..CHUNK_SIZE {
                        ...
                        let global_x = (cx * CHUNK_SIZE as i32 + x as i32) as f32;
                        let global_z = (cz * CHUNK_SIZE as i32 + z as i32) as f32;
                        let height =
                            (16.0 + (global_x * 0.1).sin() * 4.0 + (global_z * 0.1).cos() * 4.0)
                                as usize;
        ...
        let chunk_positions: Vec<IVec3> = world.chunks.keys().cloned().collect();
        let chunk_meshes_data: Vec<(IVec3, mesher::Mesh)> = chunk_positions
            .par_iter()
            .filter_map(|pos| {
                if let Some(chunk) = world.get_chunk(pos) {
                    let mesh = mesher::generate_mesh(chunk, &world);
        ...
        for (pos, mesh) in chunk_meshes_data {
    ...
    pub fn input_mouse(&mut self, dx: f64, dy: f64) {
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
            ...
            });
        {
            ...
            for (vertex_buffer, index_buffer, num_indices) in self.chunk_meshes.values() {
