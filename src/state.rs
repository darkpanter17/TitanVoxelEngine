...
use crate::{mesher::{self, Vertex, Mesh}, chunk::{Chunk, CHUNK_SIZE}, texture, camera};
use glam::{IVec3, Vec3};
use rayon::prelude::*;
...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            for x in 0..CHUNK_SIZE {
                for z in 0..CHUNK_SIZE {
                    ...
                    let height = (f32::sin(world_x / 10.0) * 5.0 + f32::cos(world_z / 10.0) * 5.0 + 15.0) as usize;
        ...
        let meshes: Vec<Mesh> = chunks.par_iter().map(|chunk| mesher::generate_mesh(chunk)).collect();
