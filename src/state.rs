...
impl<'a> State<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        // --- CHUNK DATA ---
        let grid_size = 3;
        let mut chunks: Vec<Chunk> = Vec::with_capacity((grid_size * grid_size) as usize);
        let offset = grid_size / 2;

        for x in -offset..=offset {
            for z in -offset..=offset {
                chunks.push(Chunk::new(IVec3::new(x, 0, z)));
            }
        }
        ...
        chunks.par_iter_mut().for_each(|chunk| {
            ...
            for x in 0..CHUNK_SIZE {
                for z in 0..CHUNK_SIZE {
                    ...
                    // Apply sine wave based terrain height
                    let base_height = 16.0;
                    let wave1 = (world_x as f32 / 10.0).sin() * 5.0;
                    let wave2 = (world_z as f32 / 10.0).cos() * 5.0;
                    let height = (base_height + wave1 + wave2) as usize;
                    let clamped_height = height.min(CHUNK_HEIGHT - 1);

                    for y in 0..=clamped_height {
                        let id = if y == clamped_height { 1 } else { 2 }; // 1=Grass (Top), 2=Dirt (Bottom)
        ...
        for mesh in meshes {
            ...
            all_indices.extend(mesh.indices.into_iter().map(|idx| idx + vertex_count));
        }
