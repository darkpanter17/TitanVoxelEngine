...
mod tests {
    ...
    fn test_voxel_indexing() {
        ...
        assert_eq!(chunk.get_voxel(x, y, z), id);

        // Comprobar que los valores vecinos no se alteran
        assert_eq!(chunk.get_voxel(x + 1, y, z), 0);
        assert_eq!(chunk.get_voxel(x, y + 1, z), 0);
        assert_eq!(chunk.get_voxel(x, y, z + 1), 0);

        // Test de limites (out of bounds) - debería retornar 0
